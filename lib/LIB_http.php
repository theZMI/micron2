...
define("COOKIE_FILE", BASEPATH . "tmp/cookie__a1u32BCasdfashd.txt");
...
function http_get_json($target, $ref)
{
    return http_json($target, $ref, $method = "GET", $data_array = "", EXCL_HEAD);
}
...
function http_post_json($target, $ref, $data_array)
{
    return http_json($target, $ref, $method = "POST", $data_array, EXCL_HEAD);
}
...
function http_json($target, $ref, $method, $data_array, $incl_head)
{
    # Initialize PHP/CURL handle
    $ch = curl_init();
    ...
    if ($method == HEAD) {
        ...
        if ($method == POST) {
            if (count($data_array)) {
                $data_str = json_encode($data_array);
