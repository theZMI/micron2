<div class="container-fluid mb-4">
    <div class="row">
        <div class="col">
            <h1 class="mt-4 mb-4">
                <span class="pull-start me-3">Типы пользователей</span>
                <a href="<?= SiteRoot('_admin/user_roles/add_or_edit') ?>" class="btn btn-primary rounded-pill pull-end">
                    ...
                    <?php if (count($list)): ?>
                        ...
                        <?php foreach ($list as $id => $v): ?>
                            <tr>
                                ...
                                <td>
                                    <ol class="mb-0 ps-3">
                                        <?php foreach (array_filter($allAccessRules, fn($r) => $v->getAccessRule($r->name)) as $r): ?>
                                            <li><?= $r->description ?></li>
                                        <?php endforeach; ?>
                                    </ol>
                                </td>
                                <td width="1%" class="text-center">
                                    <a href="<?= SiteRoot("_admin/user_roles/add_or_edit&id={$id}") ?>" class="btn btn-sm btn-primary rounded-pill" title="Изменить данные"><i class="bi bi-pencil-fill"></i></a>
                                </td>
                                <td width="1%" class="text-center">
